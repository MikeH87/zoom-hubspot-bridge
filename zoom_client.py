...
async def stream_recording_bytes(download_url: str):
    """
    TRUE streaming download from Zoom recording download_url using OAuth.

    Returns (client, cm, resp) where:
      - client is the httpx.AsyncClient
      - cm is the async context manager from client.stream(...)
      - resp is the streamed Response (use resp.aiter_bytes()).
    Caller MUST close both: await cm.__aexit__(...) then await client.aclose().
    """
    token = await get_zoom_access_token()
    ...
    async def _open_stream(url, headers):
        cm = client.stream("GET", url, headers=headers)
        resp = await cm.__aenter__()
        return cm, resp

    async def _looks_not_media(resp: httpx.Response) -> bool:
        ct = (resp.headers.get("content-type") or "").lower()
        # Zoom sometimes returns HTML/JSON (login/error) instead of media
        return ("text/html" in ct) or ("application/json" in ct)

    # Attempt 1: Bearer header
    cm, resp = await _open_stream(download_url, bearer_headers)

    # Retry with access_token param if needed
    if resp.status_code in (401, 403) or await _looks_not_media(resp):
        await cm.__aexit__(None, None, None)
        alt = _with_access_token_param(download_url, token)
        cm, resp = await _open_stream(alt, {"User-Agent": "Mozilla/5.0", "Accept": "*/*"})

    # Still failing? read a tiny preview then raise (DO NOT buffer whole file)
    if resp.status_code >= 400 or await _looks_not_media(resp):
        preview = b""
        try:
        ...
        raise RuntimeError(
            ...
            f"content-type={resp.headers.get('content-type')} preview={preview[:400]!r}"
        )
